...
const SongView = lazy(() =>
  import('./content').then((module) => ({ default: module.SongView })),
)

const SongViewFooter = lazy(() =>
  import('./content').then((module) => ({
    default: module.SongViewFooter,
  })),
)
...
export const SongViewDrawer = () => {
  ...
  const [freezeSong, setFreezeSong] = useState(false)
  ...
  const [song, voroforceCell] = useMemo(() => {
    if (freezeSong) return [songRef.current, voroforceCellRef.current]
    songRef.current = activeSong
    voroforceCellRef.current = voroforce?.cells?.focused
    return [activeSong, voroforceCellRef.current]
  }, [activeSong, freezeSong])
  ...
  return (
    <Modal
      ...
      contentProps={{
        ...(!isSmallScreen && {
          onMouseEnter: () => setFreezeSong(true),
          onMouseLeave: () => setFreezeSong(false),
        }),
        className: cn(
    ...
    >
      <Suspense>{mountContent && <SongView song={song} />}</Suspense>
    </Modal>
  )
}
